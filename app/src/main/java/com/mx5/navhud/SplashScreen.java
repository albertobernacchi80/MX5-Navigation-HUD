...
public final class SplashScreen extends Screen implements androidx.lifecycle.DefaultLifecycleObserver {
    ...
    public void onStart(@NonNull androidx.lifecycle.LifecycleOwner owner) {
        // Vedi il commento equivalente in NavHudScreen.onStart(): un'eccezione in un callback
        // di ciclo di vita non intercettata chiude l'intera app.
        try {
            NavSurfaceRenderer.getInstance().start(getCarContext(), this::draw);
        } catch (Throwable t) {
            Log.e(TAG, "errore nell'avvio del disegno dello splash", t);
        }
        try {
            handler.postDelayed(goToHud, 2000);
        } catch (Throwable t) {
            Log.e(TAG, "errore nell'avvio del timer dello splash", t);
        }
    }
    ...
    public void onStop(@NonNull androidx.lifecycle.LifecycleOwner owner) {
        try {
            ...
            Log.e(TAG, "errore nella rimozione del timer dello splash", t);
        }
        try {
            NavSurfaceRenderer.getInstance().stop();
        } catch (Throwable t) {
            Log.e(TAG, "errore nell'arresto del disegno dello splash", t);
        }
    }
}
