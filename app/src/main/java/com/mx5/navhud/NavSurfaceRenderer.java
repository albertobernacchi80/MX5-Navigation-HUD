...
final class NavSurfaceRenderer {
    ...
    void start(@NonNull CarContext carContext, @NonNull DrawCallback callback) {
        ...
        // Chiamato da onStart() delle schermate (un callback di ciclo di vita: un'eccezione
        // qui, se non intercettata, risale fino a far chiudere l'intera app). getCarService()
        // in particolare dipende dall'host ed è un punto in cui un problema imprevisto
        // (host non ancora pronto, servizio non disponibile) non deve poter bloccare tutto.
        try {
            if (!registered) {
            ...
            Log.e(TAG, "impossibile registrare il callback della Surface", t);
            // Si prosegue comunque: senza Surface non si potrà disegnare finché l'host non la
            // fornisce da sé in un secondo momento, ma l'app resta viva.
        }
        epoch++;
        try {
            handler.post(new Tick(epoch));
        } catch (Throwable t) {
            Log.e(TAG, "impossibile avviare il ciclo di disegno", t);
        }
    }
