...
public final class NavGlyphRenderer {
    ...
    /**
     * Riga diagnostica sotto al messaggio "in attesa": senza questa, se l'app resta bloccata
     * su questa schermata non c'è modo di distinguere, da una semplice foto, se il problema è
     * (a) il permesso "Accesso alle notifiche" non è (più) attivo per questa app, (b) nessuna
     * notifica di Google Maps sta arrivando affatto — spesso perché le notifiche di Google Maps
     * sono disattivate a livello di sistema (Impostazioni > App > Google Maps > Notifiche),
     * un interruttore DIVERSO dal permesso "Accesso alle notifiche" concesso alla nostra app —
     * oppure (c) le notifiche arrivano ma senza un testo leggibile (formato imprevisto).
     */
    private static void drawWaitingDiagnostics(Canvas c, RectF inner, float y, float textSize) {
        ...
        boolean connected = NavHudState.isListenerConnected();
        int seen = NavHudState.mapsNotificationsSeenCount();

        p.setColor(connected ? TRACK : RED);
        String line1 = connected
                ? "Servizio notifiche: collegato"
                : "Servizio notifiche: NON collegato (controlla il permesso \"Accesso alle notifiche\")";
        c.drawText(line1, inner.centerX(), y, p);

        p.setColor(seen > 0 ? TRACK : (connected ? RED : TRACK));
        String line2 = seen > 0
                ? "Notifiche Google Maps ricevute: " + seen
                : "Notifiche Google Maps ricevute: 0 (verifica che siano attive in Impostazioni > App > Google Maps > Notifiche)";
        c.drawText(line2, inner.centerX(), y + textSize * 1.5f, p);

        if (seen > 0) {
            String rawTitle = NavHudState.lastRawTitle();
            String rawText = NavHudState.lastRawText();
            boolean empty = rawTitle.isEmpty() && rawText.isEmpty();
            p.setColor(empty ? RED : TRACK);
            String line3 = empty
                    ? "Ultima notifica: senza testo leggibile (formato imprevisto)"
                    : "Ultima notifica: \"" + truncate(rawTitle + " " + rawText, 40) + "\"";
            c.drawText(line3, inner.centerX(), y + textSize * 3f, p);
        }
    }
